...
1. **Resources**: Located in `src/pyvider/components/resources/`. These manage stateful infrastructure.
1. **Functions**: Located in `src/pyvider/components/functions/`. These are stateless utility functions.
...
1. **Install `uv`**: If you don't have it, install it with:

   ```bash
   curl -LsSf https://astral.sh/uv/install.sh | sh
   ```

1. **Create and Sync the Environment**: This command creates a virtual environment (`.venv`) and installs all required dependencies from `uv.lock`.
...
1. **Write Tests**: Add comprehensive tests for your component in the `tests/` directory, mirroring the source structure.
