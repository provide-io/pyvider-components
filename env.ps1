# env.ps1 - pyvider-components Development Environment Setup
#
# This script sets up a clean, isolated development environment for pyvider-components
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the pyvider-components root directory"
    exit 1
}
...
$Profile = if ($env:PYVIDER_COMPONENTS_PROFILE) { $env:PYVIDER_COMPONENTS_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/pyvider-components_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "pyvider-components_setup"
...
Write-Host "Installing pyvider-components in editable mode..." -NoNewline
try {
...
Write-Host "`n$("pyvider-components development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  pyvider-components --help  # pyvider-components CLI"
