...
def fix_empty_blocks(content):
    ...
    while i < len(lines):
        ...
        if stripped and stripped[-1] == ':' and any(stripped.startswith(k) for k in ['if ', 'elif ', 'else:', 'for ', 'while ', 'try:', 'except ', 'finally:', 'with ', 'def ', 'class ']):
            ...
            if i + 1 < len(lines):
                ...
                if not next_stripped or (next_stripped and len(next_line) - len(next_stripped) <= indent):
                    ...
                    if stripped.startswith('else:') or stripped.startswith('except') or stripped.startswith('finally:'):
                        fixed_lines.append(' ' * (indent + 4) + 'pass')
                    elif stripped.startswith('elif ') or stripped.startswith('if '):
                        fixed_lines.append(' ' * (indent + 4) + 'pass')
