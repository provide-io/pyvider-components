...
locals {
  ...
  resource_calculations_instances = [
  ...
  resource_calculations_monthly_costs = [
    for inst in local.resource_calculations_instances :
  ...
  resource_calculations_total_monthly_cost = provider::pyvider::sum(local.resource_calculations_monthly_costs)
  resource_calculations_average_instance_cost = provider::pyvider::divide(
    local.resource_calculations_total_monthly_cost,
    length(local.resource_calculations_instances)
  )
}
...
locals {
  ...
  resource_calculations_scale_factor = provider::pyvider::divide(
    local.resource_calculations_current_cpu_percent,
    local.resource_calculations_target_cpu_percent
  )
...
output "resource_calculations" {
  value = {
    cost_analysis = {
      total_monthly = local.resource_calculations_total_monthly_cost
      average_per_instance = local.resource_calculations_average_instance_cost
      individual_costs = local.resource_calculations_monthly_costs
    }
    storage_analysis = {
