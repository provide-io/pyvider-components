...
locals {
  ...
  comp_formatted_message = provider::pyvider::format(local.comp_template_string, [
    "Alice",
    5
  ])  # Returns: "Hello, Alice! You have 5 messages."

  # Simple template
  comp_simple_format = provider::pyvider::format("User: {}", [
    "admin"
  ])  # Returns: "User: admin"
}
...
output "string_manipulation_examples" {
  value = {
    ...
    formatting = {
      ...
      formatted = local.comp_formatted_message
      simple = local.comp_simple_format
    }
